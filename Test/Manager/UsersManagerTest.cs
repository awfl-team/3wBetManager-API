...
namespace Test.Manager
{
    ...
    internal class UsersManagerTest
    {
        ...
        private static readonly List<Bet> _betsByUser =
            JsonConvert.DeserializeObject<List<Bet>>(TestHelper.GetDbResponseByCollectionAndFileName("betsByUser"));

        private static readonly List<User> _usersOrderedByPoints =
            JsonConvert.DeserializeObject<List<User>>(TestHelper.GetDbResponseByCollectionAndFileName("users"));

        private static readonly User _user = _users[0];
        ...
        public async Task AssertThatUsernameAndEmailExistReturnsMessage(string message, User userFoundByUsername = null,
